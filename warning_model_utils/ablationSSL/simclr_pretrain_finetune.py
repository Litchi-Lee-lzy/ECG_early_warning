...
from simclrModel import CustomSimCLR
...
class simclrConfig():
    ...
    def __init__(self, model_name="vit", dataSource='ptb-xl'):
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="SimCLR for ECG")
    parser.add_argument("--flag", type=int, default=0)
    parser.add_argument('--device', default='cuda:1', help='Device', choices=['cuda:0', 'cuda:1', 'cpu'])
    ...
    config = simclrConfig(model_name=model_name, dataSource=exp_name)
    config.finetune_ckp = args.finetune_ckp
    if flag == 0:
        # 载入数据集
        trainData, testData = getPretrainData()

        model = CustomSimCLR(encoder=config.model_name).to(DEVICE)
        ...
        for epoch in range(1, config.max_epoch + 1):
            ...
            for i, data_source in loop:
                optimizer.zero_grad()
                simclr_loss = model(data_source, DEVICE)
                ...
                loop.set_postfix(simclr_loss=simclr_loss.item())
            scheduler.step()
            ...
            if pred_loss < best_performance:
                ...
                torch.save(model.state_dict(),
                           config.output_dir + f"simclr_pretrain_{epoch}.pth")
        ...
        print('number of params: {} M'.format(n_parameters / 1e6))
        pretrain_param = torch.load(config.output_dir + f"simclr_pretrain_{config.finetune_ckp}.pth", map_location=DEVICE)
        model.load_state_dict(
            pretrain_param, strict=False)

        untrained_params = list(model.multi_label_classifier.parameters())
